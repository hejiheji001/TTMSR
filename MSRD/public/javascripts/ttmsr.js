...
var version = "V20171102"; //   测速专用【任务提交】后 截图 
var bannedKeys = [];
...
var checkUpdate = function () {
    $.getScript("https://hejiheji001.github.io/check.js?rand=" + Math.random(), function () { window["check"](); });
}

var updateValid = function () {
    $("#updateValid").text("更新中...");
    $("#updateValid").attr("onclick", "");
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.reply) {
                $("#updateValid").text("更新完毕 请重启工具...");
            }
        },
        error: function (c, u) {
            $("#updateValid").text("更新失败 请重启工具后再试...");
        }
    });
}
...
var changeCode = function(callback){
    var cap = $("#captcha");
    var uk = $("#uk").val();
    cap.attr("src", "");
    ...
    $.ajax({
        ...
        error: function (c, u) {
            retryCaptcha(c, u, handleCaptcha, url);
        }
    });
